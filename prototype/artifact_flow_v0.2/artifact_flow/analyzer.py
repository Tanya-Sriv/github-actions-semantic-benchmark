...
def canonical_condition(value: str | None) -> str | None:
    """Normalize only superficial condition syntax for exact-equivalence checks."""
    if value is None:
        return None
    text = normalize_condition(value)
    if text is None:
        return None
...
def producer_is_conditionally_non_guaranteed(
    producer: ArtifactEndpoint,
    consumer: ArtifactEndpoint,
    triggers: set[str],
) -> tuple[bool, str]:
    condition = canonical_condition(producer.condition)
    consumer_condition = canonical_condition(consumer.condition)
    if not condition:
        return False, "unconditional producer"
    if condition in STATUS_FUNCS or condition == "true":
        return False, "default/status condition"

    # v0.2 refinement: exactly equivalent producer and consumer conditions are
    # treated as correlated. The consumer cannot execute on a path where the
    # producer is skipped by that same condition.
    if consumer_condition is not None and condition == consumer_condition:
        return False, "producer and consumer share an equivalent condition"

    if condition == "false":
        return True, "producer condition is false"

    # Narrow, explainable contradictions and conditionality checks.
    ref_eq = re.fullmatch(r"github\.ref\s*==\s*['\"]refs/heads/([^'\"]+)['\"]", condition)
    if ref_eq and "push" not in triggers:
        return True, "producer requires a branch ref but workflow has no push trigger"

    event_eq = re.fullmatch(r"github\.event_name\s*==\s*['\"]([^'\"]+)['\"]", condition)
...
def analyze_workflow(path: Path) -> dict[str, Any]:
    ...
    for consumer in consumers:
        ...
        cross_job_matching = [p for p in matching if p.job in ancestors]
        same_job_preceding = [
            p for p in matching
            if p.job == consumer.job and p.step_index < consumer.step_index
        ]
        dependency_matching = cross_job_matching + same_job_preceding
        if not dependency_matching:
            ...
            findings.append(Finding(
                rule_id="AF002",
                message=(
                    f'Job {consumer.job!r} consumes artifact {name!r}, but no matching '
                    "producer is guaranteed to execute earlier through either same-job "
                    "step order or the consumer job's declared needs ancestry."
                ),
                workflow=str(path),
                consumer_job=consumer.job,
                consumer_step_index=consumer.step_index,
                ...
                confidence="high",
                evidence={
                    "reason": "missing_declared_dependency_or_prior_same_job_producer",
                    "consumer_needs_ancestors": sorted(ancestors),
                    "same_job_late_producer_steps": sorted(p.step_index for p in same_job_late),
                },
            ))
            continue
        ...
        for producer in dependency_matching:
            non_guaranteed, reason = producer_is_conditionally_non_guaranteed(producer, consumer, triggers)
            if non_guaranteed:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Artifact-Flow Analysis prototype v0.2")
